        ...
        <div className="flex items-center gap-3">
          ...
          {selectedCount > 0 && (
            <div className="flex items-center gap-1 border-l border-[#1A1A1A]/10 pl-3">
              <button onClick={onBulkArchive} className="p-1 px-2 border border-[#1A1A1A]/10 hover:bg-[#1A1A1A]/5 text-[9px] font-mono tracking-widest uppercase flex items-center gap-1">
                <Archive className="w-3 h-3" /><span>{lang === 'zh' ? '归档' : 'ARCHIVE'}</span>
              </button>
              <button onClick={onBulkDelete} className="p-1 px-2 border border-red-200 hover:bg-red-50 text-red-800 text-[9px] font-mono tracking-widest uppercase flex items-center gap-1">
                <Trash2 className="w-3 h-3" /><span>{lang === 'zh' ? '丢弃' : 'DELETE'}</span>
              </button>
              <button onClick={() => onBulkMarkRead(true)} className="p-1 px-2 border border-[#1A1A1A]/10 hover:bg-[#1A1A1A]/5 text-[9px] font-mono tracking-widest uppercase">
                <span>{lang === 'zh' ? '标已读' : 'READ'}</span>
              </button>
              <button onClick={() => onBulkMarkRead(false)} className="p-1 px-2 border border-[#1A1A1A]/10 hover:bg-[#1A1A1A]/5 text-[9px] font-mono tracking-widest uppercase">
        ...
        <div className="grid grid-cols-4 border-b border-[#1A1A1A]/10 text-xs text-center font-mono uppercase bg-neutral-50/50">
          {TABS.map(({ key, icon, label, sub }) => (
            ...
            >
              ...
              <span className="text-[8px] opacity-40">{sub}</span>
            </button>
          ))}
        </div>
      )}
              ...
              >
                <div className="flex items-center gap-3 shrink-0" onClick={e => e.stopPropagation()}>
                  ...
                  <button onClick={e => onToggleImportant(m.id, e)} title={lang === 'zh' ? '标识等级' : 'Audit Tier Indicator'}>
                    <span className={`text-[8px] font-mono uppercase tracking-tight px-1 font-bold ${m.important ? 'bg-[#1A1A1A] text-white border border-[#1A1A1A]' : 'border border-[#1A1A1A]/10 text-[#1A1A1A]/30'}`}>
                      {m.important ? '★ IMP' : 'std'}
                    </span>
                  </button>
                </div>
                ...
                <div className="flex-1 min-w-0 pr-4">
                  <div className="flex items-center gap-2">
                    {currentFolder === 'snoozed' && m.snoozedUntil && (
                      <span className="bg-amber-100 text-amber-900 border border-amber-200 text-[8px] font-mono px-1 font-bold lowercase">⌚ {m.snoozedUntil}</span>
                    )}
                    <span className="truncate text-stone-900 font-medium block">{m.subject}</span>
                  </div>
                  <span className="font-sans text-[11px] text-neutral-400 truncate group-hover:text-stone-500 block">{m.snippet}</span>
                </div>
