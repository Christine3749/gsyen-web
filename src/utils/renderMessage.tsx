...
export function renderMessageContent(
  ...
  if (!stripped) return trailingInline ? [<span key="stream-caret-only">{trailingInline}</span>] : [];
  ...
  const tail = (i: number) => (i === trailingIndex ? trailingInline : null);
  return lines.map((line, i) => {
    ...
    if (line.trim().startsWith('> ')) {
      return (
        <div key={i} className={`border-l-2 pl-3 my-1.5 font-sans text-xs leading-relaxed ${isAI ? 'border-[#CCCCCC] text-[#999999]' : 'border-white/30 text-white/45'}`}>
          {parseBoldText(line.trim().slice(2), isAI)}{tail(i)}
        </div>
      );
    }
    // Bullet list
    ...
    if (numMatch) {
      return (
        <div key={i} className={`ml-3 my-1.5 flex gap-2 font-sans text-xs leading-relaxed ${isAI ? 'text-[#2F2F2F]' : 'text-white/95'}`}>
          ...
          <span className="flex-1">{parseBoldText(numMatch[2], isAI)}{tail(i)}</span>
        </div>
      );
    }
    // H3
    if (line.trim().startsWith('### ')) {
      return <h4 key={i} className={`text-xs font-mono font-bold uppercase tracking-wider mt-4 mb-2 ${isAI ? 'text-[#1A1A1A]' : 'text-white'}`}>{line.trim().slice(4)}{tail(i)}</h4>;
    }
    // H2
    if (line.trim().startsWith('## ')) {
      return <h3 key={i} className={`text-sm font-serif font-bold italic mt-5 mb-2.5 ${isAI ? 'text-[#1A1A1A]' : 'text-[#F9F8F6]'}`}>{line.trim().slice(3)}{tail(i)}</h3>;
    }
    // Empty line
    ...
    return (
      <p key={i} className={`leading-relaxed font-sans text-xs my-1 ${isAI ? 'text-[#2F2F2F]' : 'text-white'}`}>
        {parseBoldText(line, isAI)}{tail(i)}
      </p>
    );
  });
}
